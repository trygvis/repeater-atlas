...
async fn run_case(
    ...
    let result = c.transaction(move |c| {
        Box::pin(async move {
            ...
            for (left_key, right_key) in links.iter() {
                let left = call_signs
                    .get(left_key)
                    .ok_or_else(|| TestError::Other(format!("missing call sign for {left_key}")))?;
                let right = call_signs
                    .get(right_key)
                    .ok_or_else(|| TestError::Other(format!("missing call sign for {right_key}")))?;
