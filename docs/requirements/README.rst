Repeater Atlas Requirements
===========================

This document assembles the implemented requirements extracted from the design
documents. Each requirement is stored as a standalone file in this directory and
included below in chapter order.

.. contents:: Chapters
   :local:
   :depth: 2

Application Scope
-----------------

These requirements describe the public application scope, deployment model, and
cross-cutting public behavior.

.. include:: ra-1000.rst

.. include:: ra-1001.rst

.. include:: ra-1002.rst

.. include:: ra-1003.rst

.. include:: ra-1004.rst

Club Views and Geocoding
------------------------

These requirements cover club-scoped public views and optional address
geocoding behavior.

.. include:: ra-1005.rst

.. include:: ra-1006.rst

.. include:: ra-1007.rst

Authentication and Authorization
--------------------------------

These requirements describe account identity, JWT-cookie authentication, signup,
logout, validation, and role lookup.

.. include:: ra-1008.rst

.. include:: ra-1009.rst

.. include:: ra-1010.rst

.. include:: ra-1011.rst

.. include:: ra-1012.rst

.. include:: ra-1013.rst

.. include:: ra-1014.rst

Call Sign Search
----------------

These requirements describe the public call sign search experience.

.. include:: ra-1015.rst

.. include:: ra-1016.rst

.. include:: ra-1017.rst

.. include:: ra-1018.rst

Repeater Data Model
-------------------

These requirements describe repeater system identity, services, coordinates, and
linking.

.. include:: ra-1019.rst

.. include:: ra-1020.rst

.. include:: ra-1021.rst

.. include:: ra-1022.rst

.. include:: ra-1023.rst

.. include:: ra-1024.rst

Exports
-------

These requirements describe shared export loading and CHIRP CSV behavior.

.. include:: ra-1025.rst

.. include:: ra-1026.rst

.. include:: ra-1027.rst

.. include:: ra-1028.rst

.. include:: ra-1029.rst

Map Page
--------

These requirements describe the primary public map page.

.. include:: ra-1030.rst

.. include:: ra-1031.rst

.. include:: ra-1032.rst

.. include:: ra-1033.rst

.. include:: ra-1034.rst

.. include:: ra-1035.rst

.. include:: ra-1036.rst

.. include:: ra-1037.rst

.. include:: ra-1038.rst

.. include:: ra-1039.rst

User Account Page
-----------------

These requirements describe authenticated user account features.

.. include:: ra-1040.rst

.. include:: ra-1041.rst

.. include:: ra-1042.rst

.. include:: ra-1043.rst

.. include:: ra-1044.rst

Repeater Detail Page
--------------------

These requirements describe the public repeater detail page.

.. include:: ra-1045.rst

.. include:: ra-1046.rst

.. include:: ra-1047.rst

.. include:: ra-1048.rst

.. include:: ra-1049.rst

Admin and Validation
--------------------

These requirements describe admin editing and core validation behavior.

.. include:: ra-1050.rst

.. include:: ra-1051.rst
