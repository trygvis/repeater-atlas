...
- `authentication.md <authentication.md>`_ - Auth framework options and current
  JWT-cookie implementation
- `export.md <export.md>`_ - Export architecture and CHIRP CSV field mapping
- `repeater.md <repeater.md>`_ - Repeater system data model, services, and
  linking
- `search.md <search.md>`_ - Call sign search UX, constraints, and result model

Page-level specs live under `pages/ <pages/README.rst>`_.
...
- Admin login and role-based access with manual role assignment.
...
- Search and filters by location, band, mode, and similar fields.
...
- Coordinates are stored and serialized as lat/lon; internal code uses a
  ``Point`` type to represent locations.
...
- ``/$CALL_SIGN`` call sign resolver (repeater -> detail page, contact -> org
  page).
...
The map page (``/``) accepts optional ``lat``, ``lon``, and ``zoom`` query
parameters to center the map on a specific location. These take priority over
the persisted viewport. Example: ``/?lat=63.43&lon=10.40&zoom=12``. Used by
"Show on map" links from the user locations section.
...
When saving/importing a repeater that has ``address`` set but no
``maidenhead``, the service can optionally use Nominatim to geocode the address
to lat/lon, derive Maidenhead from that, and persist all three fields.
...
- The app uses a ``bb8`` pool on top of ``diesel-async`` for PostgreSQL
  connections.
...
- For environments that need a custom CA bundle, use ``SSL_CERT_FILE`` or
  ``SSL_CERT_DIR`` in the process environment so ``rustls-native-certs`` picks
  them up when building the PostgreSQL TLS client.
...
- Authentication: JWT cookie (stateless). See
  `authentication.md <authentication.md>`_.
