...
## Purpose
  ...
  - Show ownership/contact context, linked repeaters, and nearby repeaters on a map.

## Routes

  - `/repeater/{call_sign}` renders this page.
  - `/call-sign/{call_sign}` resolves to this page when the call sign is a repeater.
...
## Data sources
  ...
  - `dao::repeater_system::select_within_radius` provides map markers within 50 km.
...
## Page sections
  ...
  - Details:
    ...
    - Maidenhead locator and lat/long (resolved from stored coords or Maidenhead).
  - Services:
...
## Behavior
  ...
  - Maidenhead/location defaults to `-` when missing.
